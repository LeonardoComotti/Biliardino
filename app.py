...
from database import (
    ...
    get_player_ranking_stats, get_tournament_progressive_standings
)
...
if page == "🏆 Torneo":
    ...
    if st.session_state.step == 2:
        ...
        with col_btn1:
            if all_valid and len(st.session_state.results) == len(schedule):
                if st.button("📊 Finalizza classifica", use_container_width=True):
    ...
    if st.session_state.step == 3:
        ...
        if standings:
            ...
            with col1:
                if st.button("💾 Salva torneo", use_container_width=True):
                    tid = save_tournament(
                        st.session_state.players,
                        st.session_state.schedule,
                        st.session_state.results,
                    ...
                    st.success(f"✅ Torneo salvato con ID: {tid}")

            with col2:
                if st.button("📝 Modifica risultati", use_container_width=True):
                    st.session_state.step = 2
                    st.rerun()

            with col3:
                if st.button("🔄 Nuovo torneo", use_container_width=True):
...
if page == "👥 Gestisci Giocatori":
    ...
    with tab1:
        ...
        if st.button("✅ Aggiungi giocatore", type="primary", use_container_width=True):
            if nome and cognome and soprannome:
                try:
                    add_player(nome, cognome, soprannome, str(data_nascita))
    ...
    with tab2:
        ...
        if all_players:
            ...
            for p in all_players:
                ...
                if info:
                    nome, cognome, soprannome, data_nascita = info
            ...
            if selected_player:
                ...
                st.button("◀ Torna all'elenco", use_container_width=True, key="back_to_list", on_click=reset_player_selection)  
                st.markdown("---")
                ...
                if player_info:
                    nome, cognome, _, data_nascita = player_info
                    ...
                    if overall_stats and overall_stats[0] > 0:
                        ...
                        if player_tournament_stats:
                            ...
                            # Average points per tournament line graph
                            st.markdown("#### 📈 Media Punti per Torneo")
                            ...
                            ax3.set_title('Media Punti per Partita', fontsize=10, fontweight='bold')
                            ax3.set_xlabel('ID Torneo', fontsize=9)
                            ax3.set_ylabel('Punti Medi per Partita', fontsize=9)
                        ...
                        st.info("Questo giocatore non ha ancora partecipato a tornei")
        else:
    ...
    with tab3:
        ...
        if all_players:
            selected_player = st.selectbox("Seleziona giocatore", all_players, key="edit_select")
            ...
            if selected_player:
                ...
                if info:
                    ...
                    col_nome, col_cognome = st.columns(2)
                    with col_nome:
                        new_nome = st.text_input("Nome", value=nome, key="edit_nome")
                    with col_cognome:
                        new_cognome = st.text_input("Cognome", value=cognome, key="edit_cognome")
                    
                    col_soprannome, col_data = st.columns(2)
                    with col_soprannome:
                        new_soprannome = st.text_input("Soprannome", value=soprannome, key="edit_soprannome")
                    with col_data:
                        new_data = st.date_input("Data di nascita", value=None if not data_nascita else None, key="edit_data")
                    
                    if st.button("✅ Salva modifiche", type="primary", use_container_width=True):
                        try:
                            update_player(soprannome, new_nome, new_cognome, new_soprannome, str(new_data))
    ...
    with tab4:
        ...
        if all_players:
            ...
            if info:
                ...
                if st.button("❌ Elimina permanentemente", type="secondary", use_container_width=True):
                    try:
...
if page == "📊 Storico Tornei":
    ...
    with tab1:
        ...
        if tournaments:
            ...
            st.dataframe(tournament_data, use_container_width=True, hide_index=True)
            ...
            if selected_tournament:
                ...
                if stats:
                    ...
                    # Progressive standings graph
                    st.markdown("#### 📊 Andamento Punti")
                    ...
                    if progressive_standings:
                        ...
                        all_players = set()
                        for match_standings in progressive_standings.values():
                            all_players.update(match_standings.keys())
                        
                        # Sort players by final points for consistent colors
                        final_standings = progressive_standings[max(progressive_standings.keys())]
                        sorted_players = sorted(final_standings.keys(), 
                                              key=lambda x: final_standings[x]["pt"], 
                                              reverse=True)
                        ...
                        # Plot each player's cumulative points over time
                        match_numbers = list(progressive_standings.keys())
                        
                        for i, player in enumerate(sorted_players):
                            points = []
                            for match_num in match_numbers:
                                # Get cumulative points for this player at this match
                                match_standings = progressive_standings[match_num]
                                if player in match_standings:
                                    points.append(match_standings[player]["pt"])
                                else:
                                    points.append(0)
                            
                            color = colors[i % len(colors)]
                            ax.plot(match_numbers, points, 'o-', linewidth=2, markersize=6, 
                                   color=color, label=player, markerfacecolor=color, 
                                   markeredgecolor='white', markeredgewidth=1)
                        
                        ax.set_title(f'Andamento Punti - Torneo {selected_tournament}', 
                                   fontsize=16, fontweight='bold', pad=20)
                        ax.set_xlabel('Partita', fontsize=14)
                        ax.set_ylabel('Punti Totali', fontsize=14)
                        ax.grid(True, alpha=0.3)
            ...
            for player in all_players:
                ...
                if overall and overall[0] > 0:  # Only show players who played tournaments
                    ...
                    global_stats.append({
                        ...
                        "📊 Punti Medi": round(punti_med, 1) if punti_med else 0,
                        "⚽ Partite Medie": round(partite_med, 1) if partite_med else 0,
                        "✅ Vittorie Medie": round(vittorie_med, 1) if vittorie_med else 0
                    })
            
            if global_stats:
                # Sort by total points
                global_stats.sort(key=lambda x: x["🏆 Punti Totali"], reverse=True)
